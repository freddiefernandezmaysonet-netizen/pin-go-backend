...
const STAGING_REVISION = "booking-search-staging-v3";
...
app.get("/api/public-booking/catalog", async (_req, res) => {
  try {
    const items = await prisma.property.findMany({
      ...
      select: { name: true, slug: true, publicTitle: true, publicPhotos: true, maxGuests: true, city: true, region: true, country: true, organization: { select: { slug: true } } },
      orderBy: { createdAt: "asc" },
      take: 200,
    });
    return res.json({ ok: true, revision: STAGING_REVISION, results: items.map((property) => ({ organizationSlug: property.organization.slug, propertySlug: property.slug, title: property.publicTitle?.trim() || property.name, city: property.city ?? null, region: property.region ?? null, country: property.country ?? null, maxGuests: property.maxGuests ?? null, photoUrl: Array.isArray(property.publicPhotos) && typeof property.publicPhotos[0] === "string" ? property.publicPhotos[0] : null, bookingPath: property.organization.slug && property.slug ? `/book/${property.organization.slug}/${property.slug}` : null })) });
  } catch (error) {
...
app.listen(PORT, async () => {
  ...
  try {
    const eligible = await prisma.property.findMany({
      where: { status: "ACTIVE", isPublicBookable: true, slug: { not: null }, organization: { publicBookingEnabled: true, slug: { not: null } } },
      select: { slug: true, publicTitle: true, name: true, city: true, region: true, organization: { select: { slug: true } } },
      take: 200,
    });
    console.log("[booking-search-staging] eligible-public-catalog", { count: eligible.length, properties: eligible.map((property) => ({ organizationSlug: property.organization.slug, propertySlug: property.slug, title: property.publicTitle?.trim() || property.name, city: property.city, region: property.region })) });
  } catch (error) {
