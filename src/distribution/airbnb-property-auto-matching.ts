...
function scoreCandidate(
  ...
  const preferredName = property.publicTitle?.trim() || property.name;
  const nameSimilarity = airbnbPropertyNameSimilarity(preferredName, listing.title);
  ...
  const listingMaxGuests = listing.occupancies?.length
    ...
    : null;
  if (property.maxGuests != null && listingMaxGuests != null) {
    if (property.maxGuests === listingMaxGuests) {
      score += 5;
