...
function normalizeDelivery(delivery: ChannexAriDispatchDelivery) {
...
function normalizePropertyState(state: ChannexAriDispatchPropertyState) {
  return {
...
function getKindNextAllowedAt(input: {
  messageKind: ChannexAriMessageKind;
  propertyState: ReturnType<typeof normalizePropertyState>;
}): Date | null {
...
export function getChannexAriDispatchWakeAt(input: {
  ...
  const eligibility = evaluateChannexAriDispatchEligibility(input);

  if (eligibility.eligible) return input.now;
