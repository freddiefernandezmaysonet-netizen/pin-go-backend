...
const VALID_ENV: NodeJS.ProcessEnv = {
  CHANNEX_DEMO_WEBHOOK_CANARY_CONFIRMATION: "RUN_CHANNEX_DEMO_WEBHOOK_CANARY",
...
function parseLog(entries: unknown[]) {
...
function dbMock(args: {
  ...
  return {
    ...
    reservation: { count: async () => counts.shift() ?? 7 },
  };
}
...
test("blocks before database or network work without explicit confirmation", async () => {
  ...
  const code = await runChannexDemoWebhookCanary(
    { ...VALID_ENV, CHANNEX_DEMO_WEBHOOK_CANARY_CONFIRMATION: "" },
    {
      prisma: {
        pmsListing: {
          findMany: async () => {
      ...
      log: () => undefined,
      logError: (v) => errors.push(v),
    }
  );
  assert.equal(code, 1);
  assert.equal(dbCalled, false);
  assert.equal(fetchCalled, false);
  const output = parseLog(errors);
...
test("blocks app.channex.io before database or network work", async () => {
  ...
  const errors: unknown[] = [];
  const code = await runChannexDemoWebhookCanary(
    { ...VALID_ENV, CHANNEX_API_BASE_URL: "https://app.channex.io" },
    {
      prisma: {
        pmsListing: {
          findMany: async () => {
  ...
  assert.equal(dbCalled, false);
  assert.equal(fetchCalled, false);
  assert.equal(parseLog(errors).status, "FAILED_SAFE");
});

test("requires exactly one active Pin&Go Demo Property listing", async () => {
  let fetchCalled = false;
  const errors: unknown[] = [];
  const code = await runChannexDemoWebhookCanary(VALID_ENV, {
    prisma: dbMock({ listings: [] }),
    fetch: (async () => {
      fetchCalled = true;
      return jsonResponse({});
    }) as any,
...
test("registers and verifies only the demo webhook then restores local and remote state", async () => {
  ...
  const fetchImpl = (async (url: string, init?: RequestInit) => {
    ...
    if (url.endsWith("/api/v1/webhooks") && init?.method === "POST") {
      return jsonResponse(webhookResponse(), 201);
    }
  ...
  assert.deepEqual(
    ...
    [
      { url: "https://staging.channex.io/api/v1/webhooks", method: "POST" },
      {
  ...
  assert.deepEqual((connectionUpdates[1] as any).data, { webhookSecret: null });
  ...
  assert.equal(publicOutput.includes("channex-secret-key"), false);
  assert.equal(publicOutput.includes("temporary-secret"), false);
});

test("verification failure deletes remote webhook without unnecessary local restore writes", async () => {
  const connectionUpdates: unknown[] = [];
  const listingUpdates: unknown[] = [];
  const errors: unknown[] = [];
  ...
  const fetchImpl = (async (url: string, init?: RequestInit) => {
    ...
    if (url.endsWith("/api/v1/webhooks/webhook-001") && init?.method === "GET") {
      const bad = webhookResponse();
      bad.data.attributes.callback_url = "https://wrong.example.com/webhooks/channex";
  ...
  const code = await runChannexDemoWebhookCanary(VALID_ENV, {
    prisma: dbMock({ connectionUpdates, listingUpdates }),
    fetch: fetchImpl,
    generateSecret: () => "temporary-secret",
    disconnect: async () => undefined,
    log: () => undefined,
  ...
  const output = parseLog(errors);
  assert.equal(output.status, "FAILED_SAFE");
  assert.equal(output.cleanup.remoteWebhookDeleted, true);
  assert.equal(connectionUpdates.length, 0);
  assert.equal(listingUpdates.length, 0);
  assert.deepEqual(calls.map((x) => x.method), ["POST", "GET", "DELETE"]);
});
