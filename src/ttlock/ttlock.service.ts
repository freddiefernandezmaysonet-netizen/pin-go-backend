...
export async function ttlockGetAccessToken(): Promise<TTLockTokenResponse> {
  ...
  if (!client_id || !client_secret || !username || !passwordPlain) {
    throw new Error(
      "Missing TTLOCK env vars (TTLOCK_CLIENT_ID/TTLOCK_CLIENT_SECRET/TTLOCK_USERNAME/TTLOCK_PASSWORD_PLAIN)"
    );
  }
  ...
  const resp = await fetch(`${base}/oauth2/token`, {
    ...
    body: new URLSearchParams({
      ...
      client_secret,
      username,
      password: md5Lower32(passwordPlain),
    }).toString(),
  });
