...
cleaningConfirmRouter.get("/cleaning/confirm/:token", async (req, res) => {
  try {
    ...
    const propertyName =
      confirmation.reservation?.property?.name ?? "Property";

    const staffName =
      confirmation.staffMember?.fullName ?? "Cleaner";
    ...
    return res.status(500).send(
      e?.message ?? "Failed to load confirmation."
    );
  }
});
...
cleaningConfirmRouter.post(
  ...
  async (req, res) => {
    try {
      ...
      const lock = confirmation.reservation.property?.locks?.find(
