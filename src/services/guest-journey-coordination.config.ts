...
function parseActivation(
  ...
  throw new Error(
    "GUEST_JOURNEY_COORDINATION_INTENTS_ENABLED_INVALID: expected true/false, 1/0, yes/no, or on/off"
  );
}
...
export function resolveGuestJourneyCoordinationConfig(
  env: NodeJS.ProcessEnv = process.env
): GuestJourneyCoordinationConfig {
  const enabled = parseActivation(
    env.GUEST_JOURNEY_COORDINATION_INTENTS_ENABLED
  );
  const organizationIds =
    parseIdentifierList(
      env.GUEST_JOURNEY_COORDINATION_INTENTS_ORGANIZATION_IDS
    );
  ...
  return {
    ...
    batchSize: parseInteger({
      ...
      rawValue:
        env.GUEST_JOURNEY_COORDINATION_INTENTS_BATCH_SIZE,
      fallback: 10,
      min: 1,
      max: 50,
    }),
    horizonDays: parseInteger({
      ...
      rawValue:
        env.GUEST_JOURNEY_COORDINATION_INTENTS_HORIZON_DAYS,
      fallback: 90,
      min: 1,
      max: 365,
    }),
    lookbackDays: parseInteger({
      ...
      rawValue:
        env.GUEST_JOURNEY_COORDINATION_INTENTS_LOOKBACK_DAYS,
      fallback: 7,
      min: 1,
      max: 30,
    }),
