...
export async function refreshNfcPoolFromTTLock(
  ...
  for (const item of list) {
    ...
    try {
      ...
      throw new Error(
        `NfcCard upsert failed for cardId=${cardId}: ${String(
          e?.message ?? e
        )}`
      );
    }
...
export async function assignNfcCards(
  ...
  if (cards.length < count) {
    throw new Error(
      `Not enough UNIQUE NFC cards. Needed=${count} found=${cards.length}`
    );
  }
...
export async function dedupeNfcCards(
  prisma: PrismaClient,
  propertyId: string
) {
  const cards = await prisma.nfcCard.findMany({
