...
test(
  "combines dispatch evidence and retryable MessageLog evidence without message bodies",
  async () => {
    const {
      tx,
      calls,
    ...
    const messageSelect =
      calls.messageLogFindMany[0]
        .select;

    assert.equal(
      "body" in messageSelect,
      false
    );
...
test(
  "rejects a missing reservation without loading communications",
  async () => {
    const {
      tx,
      calls,
