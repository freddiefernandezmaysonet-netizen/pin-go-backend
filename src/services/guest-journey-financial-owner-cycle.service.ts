...
export type FinancialOwnerCycleMetrics = {
  enabled: boolean;
  selected: number;
...
type CycleDependencies = {
  claim: typeof claimGuestJourneyFinancialIntent;
  execute: typeof executeGuestJourneyFinancialEvaluationAdapter;
  complete: typeof completeGuestJourneyFinancialIntent;
  syncMissionControl: typeof syncGuestJourneyFinancialOwnerMissionControl;
  leaseTokenFactory: () => string;
  clock: () => Date;
};

const DEFAULT_DEPENDENCIES: CycleDependencies = {
  claim: claimGuestJourneyFinancialIntent,
  execute: executeGuestJourneyFinancialEvaluationAdapter,
  complete: completeGuestJourneyFinancialIntent,
  syncMissionControl: syncGuestJourneyFinancialOwnerMissionControl,
  leaseTokenFactory: () => randomBytes(32).toString("base64url"),
  clock: () => new Date(),
};

function validateConfig(config: GuestJourneyFinancialOwnerConfig): void {
  if (config.enabled && config.organizationIds.length === 0 && config.propertyIds.length === 0) {
    throw new Error("GUEST_JOURNEY_FINANCIAL_OWNER_SCOPE_REQUIRED");
  }
}

async function selectCandidates(
  prisma: PrismaClient,
  config: GuestJourneyFinancialOwnerConfig,
  now: Date
): Promise<Array<{
  id: string;
...
export async function runGuestJourneyFinancialOwnerCycle(
  prisma: PrismaClient,
  config: GuestJourneyFinancialOwnerConfig,
  options: {
    now?: Date;
    logger?: (entry: {
      event: "GUEST_JOURNEY_FINANCIAL_OWNER_CYCLE";
      metrics: FinancialOwnerCycleMetrics;
    }) => void;
    dependencies?: Partial<CycleDependencies>;
  } = {}
): Promise<FinancialOwnerCycleMetrics> {
  const startedAt = Date.now();
  const metrics: FinancialOwnerCycleMetrics = {
  ...
  if (!config.enabled) {
    ...
    options.logger?.({ event: "GUEST_JOURNEY_FINANCIAL_OWNER_CYCLE", metrics });
    return metrics;
  }
  ...
  for (const candidate of candidates) {
    ...
    try {
      ...
      try {
        ...
        const normalized = normalizeFinancialOwnerError(error);
        completion = {
