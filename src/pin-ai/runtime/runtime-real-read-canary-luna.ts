...
async function main(): Promise<void> {
    ...
    conversation: [
      {
        ...
        content:
          reservation.preferredLanguage === "es"
            ? "¿A qué hora es mi checkout y cuál es el estado actual de mi acceso?"
            : "What time is my checkout and what is the current status of my access?",
      },
    ],
  };
  ...
  const guardedFetch = async (
    ...
    outboundCalls += 1;
    if (outboundCalls > 40) {
      throw new Error("PIN_AI_RUNTIME_REAL_READ_NETWORK_CALL_LIMIT");
    }
  ...
  console.log("PIN_AI_RUNTIME_REAL_READ_STARTED:gpt-5.6-luna");
...
main().catch(async (error: unknown) => {
  ...
  console.error(`PIN_AI_RUNTIME_REAL_READ_FAILED:${message}`);
  process.exitCode = 1;
