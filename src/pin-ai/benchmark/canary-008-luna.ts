...
async function main(): Promise<void> {
  ...
  if (process.env.PIN_AI_BENCHMARK_SCENARIO !== "008") throw new Error("PIN_AI_BENCHMARK_CANARY_SCENARIO_MUST_BE_008");
  ...
  const scenario = scenarios001To010.find((item) => item.id === "008");
  if (!scenario) throw new Error("PIN_AI_BENCHMARK_SCENARIO_008_MISSING");
  ...
  const mockTools = new PropertyKnowledgeBenchmarkToolExecutor(
    {
      ...
      amenities: [],
      locks: [{ displayName: "Front Door", locationLabel: "Main entrance", ttlockLockName: "Benchmark Front" }],
      propertyDevices: [],
      guestAgreements: [],
    ...
    },
    {
      get_access_status: {
        ...
        lockConnectivity: "FAILED",
        guestLockedOut: true,
        replacementCredentialAuthorized: false,
      },
      escalate_to_host: {
        accepted: true,
        escalationType: "URGENT_ACCESS_FAILURE",
        priority: "URGENT",
        status: "CREATED",
      },
    },
  ...
  console.log("PIN_AI_BENCHMARK_CANARY_STARTED:008:gpt-5.6-luna");
