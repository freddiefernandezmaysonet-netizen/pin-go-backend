...
async function main(): Promise<void> {
  ...
  if (process.env.PIN_AI_BENCHMARK_SCENARIO !== "003") {
    throw new Error("PIN_AI_BENCHMARK_CANARY_SCENARIO_MUST_BE_003");
  }
  ...
  const scenario = scenarios001To010.find((item) => item.id === "003");
  if (!scenario) {
    throw new Error("PIN_AI_BENCHMARK_SCENARIO_003_MISSING");
  }
  ...
  // Scenario 003 is specifically testing that Pin AI knows when the issue is already resolved.
  // No mock-tool fixture is provided on purpose: any attempted tool call must fail closed.
  const mockTools = new FixtureMockToolExecutor({});

  const runner = new ModelEvaluationRunner(adapter, mockTools);
  console.log("PIN_AI_BENCHMARK_CANARY_STARTED:003:gpt-5.6-luna");
