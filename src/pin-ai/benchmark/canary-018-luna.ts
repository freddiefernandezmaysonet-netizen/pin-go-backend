...
async function main(): Promise<void> {
  ...
  if (process.env.PIN_AI_BENCHMARK_SCENARIO !== "018") throw new Error("PIN_AI_BENCHMARK_CANARY_SCENARIO_MUST_BE_018");
  ...
  const scenario = scenarios011To020.find((item) => item.id === "018");
  if (!scenario) throw new Error("PIN_AI_BENCHMARK_SCENARIO_018_MISSING");
  ...
  const mockTools = new FixtureMockToolExecutor({
    get_reservation_context: {
      ...
      canonicalCheckOutTimeLocal: "11:00",
  ...
  console.log("PIN_AI_BENCHMARK_CANARY_STARTED:018:gpt-5.6-luna");
