...
async function main(): Promise<void> {
  ...
  if (process.env.PIN_AI_BENCHMARK_SCENARIO !== "016") throw new Error("PIN_AI_BENCHMARK_CANARY_SCENARIO_MUST_BE_016");
  ...
  const scenario = scenarios011To020.find((item) => item.id === "016");
  if (!scenario) throw new Error("PIN_AI_BENCHMARK_SCENARIO_016_MISSING");
  ...
  const mockTools = new PropertyKnowledgeBenchmarkToolExecutor(
    {
      ...
      publicDescription: "Guest-facing benchmark property. AC guidance: if the display is on and the room temperature remains above the setpoint, confirm COOL mode, allow up to 3 minutes for compressor startup, and if there is still no cooling, stop troubleshooting and escalate for maintenance.",
  ...
  console.log("PIN_AI_BENCHMARK_CANARY_STARTED:016:gpt-5.6-luna");
