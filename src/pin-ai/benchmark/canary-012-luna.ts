...
async function main(): Promise<void> {
  ...
  if (process.env.PIN_AI_BENCHMARK_SCENARIO !== "012") throw new Error("PIN_AI_BENCHMARK_CANARY_SCENARIO_MUST_BE_012");
  ...
  const scenario = scenarios011To020.find((item) => item.id === "012");
  if (!scenario) throw new Error("PIN_AI_BENCHMARK_SCENARIO_012_MISSING");
  ...
  const mockTools = new FixtureMockToolExecutor({
    check_late_checkout: {
      requestedLocalTime: "2026-09-22T12:00:00-04:00",
      available: true,
      eligible: true,
  ...
  console.log("PIN_AI_BENCHMARK_CANARY_STARTED:012:gpt-5.6-luna");
