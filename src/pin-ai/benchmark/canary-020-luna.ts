...
async function main(): Promise<void> {
  ...
  if (process.env.PIN_AI_BENCHMARK_SCENARIO !== "020") throw new Error("PIN_AI_BENCHMARK_CANARY_SCENARIO_MUST_BE_020");
  ...
  const scenario = scenarios011To020.find((item) => item.id === "020");
  if (!scenario) throw new Error("PIN_AI_BENCHMARK_SCENARIO_020_MISSING");
  ...
  const mockTools = new PropertyKnowledgeBenchmarkToolExecutor(
    {
      ...
      publicDescription: "Guest-facing benchmark property. AC guidance: the approved thermostat reset has already been exhausted; do not repeat it. If cooling is still unavailable, escalate for maintenance. Wi-Fi guidance: confirm the router power light is on; if the network remains unavailable after one power check, escalate the outage.",
      ...
      amenities: [
        ...
        {
          name: "Wi-Fi",
          description: "Guest Wi-Fi is included.",
          chargeMode: "INCLUDED",
        },
      ],
      locks: [],
    ...
    },
    {
      escalate_to_host: {
        accepted: true,
        escalationType: "MULTI_ISSUE_STAY_SUPPORT",
        priority: "URGENT",
        status: "CREATED",
        issues: ["AC_UNRESOLVED", "WIFI_OUTAGE"],
  ...
  console.log("PIN_AI_BENCHMARK_CANARY_STARTED:020:gpt-5.6-luna");
