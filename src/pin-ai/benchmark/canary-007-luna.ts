...
async function main(): Promise<void> {
  ...
  if (process.env.PIN_AI_BENCHMARK_SCENARIO !== "007") throw new Error("PIN_AI_BENCHMARK_CANARY_SCENARIO_MUST_BE_007");
  ...
  const scenario = scenarios001To010.find((item) => item.id === "007");
  if (!scenario) throw new Error("PIN_AI_BENCHMARK_SCENARIO_007_MISSING");
  ...
  const mockTools = new PropertyKnowledgeBenchmarkToolExecutor(
    ...
    },
    {
      get_reservation_context: {
        ...
        identityStatus: scenario.context.identityStatus,
        agreementsStatus: scenario.context.agreementsStatus,
      },
      get_access_status: {
        accessStatus: "SCHEDULED",
        credentialState: "NOT_RELEASED",
        releaseAuthorized: false,
        releaseAtLocal: scenario.context.accessStartsAtLocal,
      },
    },
  );

  const runner = new ModelEvaluationRunner(adapter, mockTools);
  console.log("PIN_AI_BENCHMARK_CANARY_STARTED:007:gpt-5.6-luna");
