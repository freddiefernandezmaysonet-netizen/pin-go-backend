...
async function main(): Promise<void> {
  ...
  if (process.env.PIN_AI_BENCHMARK_SCENARIO !== "002") {
    throw new Error("PIN_AI_BENCHMARK_CANARY_SCENARIO_MUST_BE_002");
  }
  ...
  const scenario = scenarios001To010.find((item) => item.id === "002");
  if (!scenario) {
    throw new Error("PIN_AI_BENCHMARK_SCENARIO_002_MISSING");
  }
  ...
  const mockTools = new FixtureMockToolExecutor({
    ...
    check_early_checkin: {
      eligible: false,
      reason: "CLEANING_IN_PROGRESS",
      scheduledCheckInLocal: scenario.context.checkInLocal,
    },
  });

  const runner = new ModelEvaluationRunner(adapter, mockTools);
  console.log("PIN_AI_BENCHMARK_CANARY_STARTED:002:gpt-5.6-luna");
