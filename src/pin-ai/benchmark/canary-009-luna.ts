...
async function main(): Promise<void> {
  ...
  if (process.env.PIN_AI_BENCHMARK_SCENARIO !== "009") throw new Error("PIN_AI_BENCHMARK_CANARY_SCENARIO_MUST_BE_009");
  ...
  const scenario = scenarios001To010.find((item) => item.id === "009");
  if (!scenario) throw new Error("PIN_AI_BENCHMARK_SCENARIO_009_MISSING");
  ...
  const mockTools = new PropertyKnowledgeBenchmarkToolExecutor(
    ...
    },
    {
      get_reservation_context: {
        ...
        canonicalCheckInTimeLocal: "16:00",
        sourceCheckInTimeClaims: {
          airbnb: "15:00",
          pinGoMessage: "16:00",
        },
        canonicalSource: "RESERVATION_CONTEXT",
      },
    },
  );

  const runner = new ModelEvaluationRunner(adapter, mockTools);
  console.log("PIN_AI_BENCHMARK_CANARY_STARTED:009:gpt-5.6-luna");
