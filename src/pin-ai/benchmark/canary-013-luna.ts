...
async function main(): Promise<void> {
  ...
  if (process.env.PIN_AI_BENCHMARK_SCENARIO !== "013") throw new Error("PIN_AI_BENCHMARK_CANARY_SCENARIO_MUST_BE_013");
  ...
  const scenario = scenarios011To020.find((item) => item.id === "013");
  if (!scenario) throw new Error("PIN_AI_BENCHMARK_SCENARIO_013_MISSING");
  ...
  const mockTools = new PropertyKnowledgeBenchmarkToolExecutor(
    {
      ...
      amenities: [
        {
          name: "Wi-Fi",
          description: "Guest Wi-Fi is available.",
          chargeMode: "INCLUDED",
        },
        {
          name: "Pool",
  ...
  console.log("PIN_AI_BENCHMARK_CANARY_STARTED:013:gpt-5.6-luna");
