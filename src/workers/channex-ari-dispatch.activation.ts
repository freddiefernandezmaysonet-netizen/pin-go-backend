export type ChannexAriDispatchActivation = {
  enabled: boolean;
...
export function resolveChannexAriDispatchActivation(
  env: NodeJS.ProcessEnv = process.env
): ChannexAriDispatchActivation {
  const rawValue = String(env.CHANNEX_ARI_DISPATCH_ENABLED ?? "").trim();
